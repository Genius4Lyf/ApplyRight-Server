...
// Keyed to the ACCOUNT, and mounted AFTER protect so req.user exists — see the module.
// Only the model-calling routes take it: /model and /company-type are plain writes.
const { conversationLimiter } = require("../middleware/rateLimit.middleware");

// Live conversational AI coach for the current builder step (free daily quota,
// paid unlimited; gated in the controller).
router.post("/guide", protect, conversationLimiter, guide);
...
router.post("/generate-bullets", protect, conversationLimiter, generateBullets);
...
router.post("/summary", protect, conversationLimiter, summary);
...
router.post("/brief", protect, conversationLimiter, getBrief);
...
router.post("/no-target", protect, conversationLimiter, setNoTarget);
...
router.post("/ask", protect, conversationLimiter, askAria);
...
router.post("/chat", protect, conversationLimiter, chat);
