const scoringService = {
  calculateFitScore: (jobAnalysis, resumeProfile) => {
    ...
    if (finalScore >= 80) {
      ...
      recommendation =
        "Good potential. You meet some key requirements. Apply, but emphasize your transferable skills.";
    } else {
      recommendation =
        "This is a reach role. Apply if you are looking to learn, but consider bridging the skill gaps first.";
    }
  ...
  generateActionPlan: (missingSkills) => {
    ...
    return missingSkills.map((skill) => ({
      skill: skill,
      action:
        actionDictionary[skill.toLowerCase()] ||
        `Complete a crash course or build a small project using ${skill}.`,
    }));
  },
};
